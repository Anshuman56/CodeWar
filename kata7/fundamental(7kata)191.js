...
function i(word) {
  //.. 
  let vowels = word.split('').filter(item => 'aioueAIOUE'.includes(item));
  let consonants = word.split('').filter(item => !'aioueAIOUE'.includes(item));
  if(word === '' || word[0] === 'I' || word[0] === word[0].toLowerCase() || vowels.length >=  consonants.length)
    return 'Invalid word';
  return 'i'+word;
}