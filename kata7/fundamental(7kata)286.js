...
function secondLargest(array){
  // Have Fun ^_^
  if(!Array.isArray(array) || array.filter(item => array.indexOf(item) === array.lastIndexOf(item)).length === 0)
    return undefined;
  
  let l = array.filter(item => typeof item === 'number'|| typeof item === 'string').map(item => +item).filter(item => !Number.isNaN(item)).sort((a, b) => a - b);
  return l[l.length - 2]
}